...
(async () => {
    ...
    // Fuzzy matching is O(n*m); oversized queries or files skip it.
    const started = Date.now();
    const longQuery = await applyReplacements('x'.repeat(10000), [{ originalText: 'y'.repeat(300), replacementText: 'z' }]);
    assert(/fuzzy matching is skipped/.test(longQuery.unsuccessfulReplacements[0]) && Date.now() - started < 1000, 'fuzzy search is skipped for a query above MAX_FUZZY_QUERY_CHARS', longQuery.unsuccessfulReplacements[0].slice(0, 120));
    const bigHaystack = await applyReplacements('x'.repeat(300 * 1024), [{ originalText: 'not here', replacementText: 'z' }]);
    assert(/fuzzy matching is skipped/.test(bigHaystack.unsuccessfulReplacements[0]), 'fuzzy search is skipped for content above MAX_FUZZY_HAYSTACK_CHARS');
    const fuzzy = await applyReplacements('const value = 1;\n', [{ originalText: 'const valu = 1;', replacementText: 'const value = 2;' }]);
    assert(fuzzy.fuzzyReplacements.length === 1 && fuzzy.updatedContent.includes('value = 2'), 'small fuzzy matches still apply', JSON.stringify(fuzzy));
})().catch((err) => {
