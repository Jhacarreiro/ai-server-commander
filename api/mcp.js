...
module.exports = function createMcpHandler(config) {
    ...
    const tool = {
        ...
        description: 'Run a shell command on the remote ServerCommander host. Use only when the user explicitly asks for remote terminal execution. Returns stdout, stderr, exit code and timeout status.',
        inputSchema: {
            type: 'object',
            properties: {
                command: {
    ...
    async function handleRequest(message) {
        ...
        if (method === 'initialize') {
            return jsonRpcResult(id, {
                ...
                instructions: 'This MCP server exposes remote terminal execution on the remote host. Use run_terminal_command only with explicit user approval and prefer short, verifiable commands.'
            });
        }
